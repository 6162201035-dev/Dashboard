...
with col1:
    with st.container(border=True, height=350): # Buat lebih tinggi
        ...
        st.page_link("pages/1_Customer_Profile.py", label="Buka ➔", use_container_width=True)

with col2:
    with st.container(border=True, height=350):
        st.markdown("### 2. 🔗 Relation")
        st.markdown("Analisis hubungan antara area yang saling berasosiasi.")
        st.page_link("pages/2_Associated_Area.py", label="Buka ➔", use_container_width=True) 

with col3:
    with st.container(border=True, height=350):
        st.markdown("### 3. 🏬 Potency")
        st.markdown("Analisis potensi perilaku pelanggan di setiap area.")
        st.page_link("pages/3_Area_Performance.py", label="Buka ➔", use_container_width=True) 

with col4:
    with st.container(border=True, height=350):
        st.markdown("### 4. ⏳ Period")
        st.markdown("Analisis pola pelanggan terhadap waktu per jam dan hari.")
        st.page_link("pages/4_Time_Period_Traffic_Flow.py", label="Buka ➔", use_container_width=True)
        
with col5:
    with st.container(border=True, height=350):
        st.markdown("### 5. 🚦 Traffic")
        st.markdown("Analisis traffic dan flow gabungan untuk Area dan Gerbang.")
        st.page_link("pages/5_Area_Traffic_Gate_Flow.py", label="Buka ➔", use_container_width=True)
